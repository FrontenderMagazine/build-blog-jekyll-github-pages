...
статических сайтов, разработанный для создания минималистичных блогов, 
которые можно разместить на GitHub Pages. Написание статей и
создание тем для Jekyll удивительно просты, однако
настройка сайта заняла у меня значительно больше времени, чем ожидалось.
...
используя сайт на простом статическом HTML. Всё содержимое сайта хранится 
на GitHub, который, к тому же, осуществляет контроль версий файлов. 
Целью было избавиться от сложности, свойственной другим блог-платформам, 
создав рабочий процесс, чтобы при этом можно было вести блог как [настоящий хакер][1].
...
* **Простота**  
  Jekyll сводит все к абсолютному минимуму, избавляясь от сложности:
  ...
  * **Быстрый**
    Jekyll быстрый, потому, что ограничен в
    возможностях и не использует базы данных — он просто
  ...
  * **Минималистичный**
    Большинство сайтов на Jekyll не содержит никаких
    лишних функций.

* **Контроль представления**
  Проводите меньше времени борясь со сложными шаблонами,
  написанными другими людьми, и больше — создавая свой
  собственный или адаптируя простой базовый шаблон.

* **Безопасность**
  Большинство уязвимостей, которые есть у платформ вроде
  WordPress, отсутствуют в Jekyll, так как здесь нет
  CMS, баз данных или PHP. Поэтому вам не нужно тратить
...
Давайте начнем с самого быстрого и простого варианта: форкнем репозиторий
...
Начнем с создания форка репозитория — это соответствует хорошим практикам, 
которые мы упоминали в статье. Этот способ направит нас по верному 
...
иметь ввиду, размещая блог на Jekyll на GitHub Pages.

**Проблема №2**: нужно понимать различия между размещением на GitHub 
[сайта пользователя и проекта][22]. Для пользовательского сайта (который 
мы делаем) не нужно создавать никакие ветки: `master` и так сконфигурирована 
нужным образом, что бы обрабатывать с помощью Jekyll всё, что в неё помещают,
...
в поддиректории. URL будет выглядеть как `http://yourname.github.io/repository-name`, 
...
Вы можете править файлы блога одним из трех способов. Выберите тот, который
...
чтобы настроить или внести изменения в сайт на Jekyll — GitHub Pages сделает 
...
комфортно залогиниться в CMS, чтобы сделать быстрые правки, находясь за чужим компьютером. Оказалось, блог на Jekyll будет иметь CMS, если использовать
...
в других CMS, — это не будет помехой даже если вы окажетесь далеко от
...
скопировать туда, где вы храните свои изображения для Jekyll-блога, 
...
Если тема, которую вы хотите создать, довольно сложная, разумно
...
на GitHub Pages, изображения будут отдаваться с CDN GitHub'а и загружаться 
очень быстро. Я пока не вижу причин хранить их в другом месте, но, если 
...
для добавления картинок в текст тоже не сложен и выглядит вот так:
...
не все из них предоставляют возможности гибких настроек и
